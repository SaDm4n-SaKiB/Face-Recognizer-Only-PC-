...
def save_a_value(a):
    with open(os.path.join(BASE_DIR, 'a_value.txt'), 'w') as f:
        f.write(str(a))

# Function to load the value of 'a' from a file
def load_a_value():
    if os.path.exists(os.path.join(BASE_DIR, 'a_value.txt')):
        with open(os.path.join(BASE_DIR, 'a_value.txt'), 'r') as f:
            return int(f.read())
    else:
...
def capture():
    ...
    try:
        ...
        # Save the captured image to the 'static' folder
        captured_image_path = os.path.join(BASE_DIR, 'static', 'captured.jpg')
        ...
        if len(captured_encoding) > 0:
            ...
            if best_confidence >= 85:
                # Redirect to success page if the best confidence value is greater than or equal to 85
                return jsonify({'redirect_url': '/recognized'})
            else:
                # Save the captured face image to the "faces" folder
                captured_image_path = os.path.join(KNOWN_FACES_DIR, f"unknown_{a}.jpg")
                a += 1
        ...
        return jsonify({'error': 'An error occurred while capturing the image'})
